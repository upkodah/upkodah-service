# grid.geogrid.py
import math
import geoconst as gcon
import geoerror as gerr
...
def literal_grid(glon, glat):
    ...
    for i in range(0,gcon.X_ID_LENGTH):
        g_literal[gcon.X_ID_LENGTH-i-1] = str(glat%10)
        glat = int(glat/10)
    # x_id 범위부터 grid_id 범위까지
    for i in range(gcon.X_ID_LENGTH, gcon.GRID_ID_LENGTH):
        g_literal[gcon.GRID_ID_LENGTH + gcon.X_ID_LENGTH - i -1] = str(glon%10)
        glon= int(glon/10)
