...
class GridIdLengthError(Exception):
    ...
    message = f'입력된 Grid ID의 길이가 유효하지 않습니다. 입력 Grid ID의 길이는 {geoconst.GRID_ID_LENGTH}자이어야 합니다.'):
...
class LatitudeRangeError(Exception):
    ...
    message = f'Latitude is not in ({geoconst.KOR_BND_LOW}, {geoconst.KOR_BND_UPP}) range'):
        self.lat = lat
        self.message = message
        super().__init__(message)
...
class LongitudeRangeError(Exception):
    ...
    message = f'Longitude is not in range ({geoconst.KOR_BND_LEFT}, {geoconst.KOR_BND_RIGHT})'):
        self.lon = lon
        self.message = message
        super().__init__(message)
