...
class GetStationByBusRoute:
    ...
    def search(self):
        ...
        for i in range(0,len(itemList_tag)):
            ...
            sectSpd.append(itemList_tag[i].find('sectSpd').text)
            fullSectDist.append(itemList_tag[i].find('fullSectDist').text)
            ...
            bus_route_info['구간속도'] = sectSpd
            bus_route_info['구간거리'] = fullSectDist
...
class DestinationStation:
    ...
    def result_station(self):
        ...
        print(bus_route)  # 정류장이 지나는 버스 노선 찾기
        ...
        for route in bus_route:
            ...
            sect_spd = list(np.array(df['구간속도'].tolist()))
            sect_dist = list(np.array(df['구간거리'].tolist()))
            station_name = list(np.array(df['정류장이름'].tolist()))
            ...
            for station in station_list:
                ...
                if station in station_num:
                    ...
                    while True:
                        # 속도를 km/h에서 m/m으로 수정
                        meter_per_min = int(sect_spd[start_index + cnt]) * 1000 / 60
                    ...
                    if bus_direction[start_index] == bus_direction[dest_index]:
                        ...
                        stt_name = station_name[dest_index]
                        one_result = [gps_x[dest_index],gps_y[dest_index], req_time, stt_name]

                        if one_result not in station_gps:
                            # 중복되는 결과 제외
                            station_gps.append(one_result)
                        '''
